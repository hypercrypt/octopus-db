...
AS SELECT
   ...
   `saving_session`.`code` AS `code`,from_unixtime(`saving_session`.`start_at`) AS `start_at`,from_unixtime(`saving_session`.`end_at`) AS `end_at`,
   `saving_session`.`points` AS `points`,floor((`saving_session`.`points` / 8)) AS `reward`
FROM `saving_session` order by from_unixtime(`saving_session`.`start_at`) desc;
...
CREATE VIEW `price_h`
AS SELECT
   from_unixtime(`price`.`valid_from`) AS `valid_from`,from_unixtime(`price`.`valid_to`) AS `valid_to`,
...
AS SELECT
   from_unixtime(`reduced_rates`.`valid_from`) AS `valid_from`,from_unixtime(`reduced_rates`.`valid_to`) AS `valid_to`,
   `reduced_rates`.`value_exc_vat` AS `value_exc_vat`,
   `reduced_rates`.`value_inc_vat` AS `value_inc_vat`,
   `reduced_rates`.`comment` AS `comment`
FROM `reduced_rates` order by from_unixtime(`reduced_rates`.`valid_to`) desc;
...
AS SELECT
   from_unixtime(`tariff`.`valid_from`) AS `valid_from`,from_unixtime(`tariff`.`valid_to`) AS `valid_to`,
...
CREATE VIEW `usage_h`
AS SELECT
   from_unixtime(`usage`.`interval_start`) AS `interval_start`,from_unixtime(`usage`.`interval_end`) AS `interval_end`,
...
AS SELECT
   cast(from_unixtime(`u`.`interval_start`) as date) AS `date`,round(sum(if((`u`.`reduced_rate_inc_vat` <= 10),`u`.`consumption`,0)),3) AS `off-peak kWh`,round(sum(if((`u`.`reduced_rate_inc_vat` > 10),`u`.`consumption`,0)),3) AS `peak kWh`,round(sum(`u`.`consumption`),3) AS `total kWh`,round((sum(`u`.`reduced_price_inc_vat`) / 100),2) AS `cost`,round((sum(`u`.`reduced_price_inc_vat`) / sum(`u`.`consumption`)),2) AS `p/kWh`,group_concat(distinct `u`.`tariff` separator ',') AS `tariffs`
FROM `usage` `u` where (`u`.`fuel` = 'electricity') group by cast(from_unixtime(`u`.`interval_start`) as date) order by cast(from_unixtime(`u`.`interval_start`) as date) desc;
...
AS SELECT
   date_format(`billing_e_day`.`date`,'%Y') AS `year`,round(sum(`billing_e_day`.`off-peak kWh`),3) AS `off-peak kWh`,round(sum(`billing_e_day`.`peak kWh`),3) AS `peak kWh`,round(sum(`billing_e_day`.`total kWh`),3) AS `total kWh`,round(sum(`billing_e_day`.`cost`),2) AS `cost`,round(((sum(`billing_e_day`.`cost`) / sum(`billing_e_day`.`total kWh`)) * 100),2) AS `p/kWh`,group_concat(distinct `billing_e_day`.`tariffs` separator ',') AS `tariffs`
FROM `billing_e_day` group by `year` order by `year` desc;
...
AS SELECT
   from_unixtime(`u`.`interval_start`) AS `time`,round(if((`u`.`reduced_rate_inc_vat` <= 10),`u`.`consumption`,0),4) AS `off-peak kWh`,round(if((`u`.`reduced_rate_inc_vat` > 10),`u`.`consumption`,0),4) AS `peak kWh`,round(`u`.`consumption`,3) AS `total kWh`,round((`u`.`reduced_price_inc_vat` / 100),6) AS `cost`,round(`u`.`reduced_rate_inc_vat`,4) AS `p/kWh`,
   `u`.`tariff` AS `tariffs`
FROM `usage` `u` where (`u`.`fuel` = 'electricity') order by `u`.`interval_start` desc;
...
AS SELECT
   date_format(`billing_e_day`.`date`,'%Y-%m') AS `month`,round(sum(`billing_e_day`.`off-peak kWh`),3) AS `off-peak kWh`,round(sum(`billing_e_day`.`peak kWh`),3) AS `peak kWh`,round(sum(`billing_e_day`.`total kWh`),3) AS `total kWh`,round(((100 * sum(`billing_e_day`.`off-peak kWh`)) / sum(`billing_e_day`.`total kWh`)),1) AS `percent off-peak`,round(sum(`billing_e_day`.`cost`),2) AS `cost`,round(((sum(`billing_e_day`.`cost`) / sum(`billing_e_day`.`total kWh`)) * 100),2) AS `p/kWh`,group_concat(distinct `billing_e_day`.`tariffs` separator ',') AS `tariffs`
FROM `billing_e_day` group by `month` order by `month` desc;
...
AS SELECT
   concat(substr(from_unixtime(`u`.`interval_start`),1,14),'00') AS `hour`,round(sum(if((`u`.`reduced_rate_inc_vat` <= 7.5),`u`.`consumption`,0)),3) AS `off-peak kWh`,round(sum(if((`u`.`reduced_rate_inc_vat` > 7.5),`u`.`consumption`,0)),3) AS `peak kWh`,round(sum(`u`.`consumption`),3) AS `total kWh`,round((sum(`u`.`reduced_price_inc_vat`) / 100),2) AS `cost`,round((sum(`u`.`reduced_price_inc_vat`) / sum(`u`.`consumption`)),2) AS `p/kWh`,group_concat(distinct `u`.`tariff` separator ',') AS `tariffs`
FROM `usage` `u` where (`u`.`fuel` = 'electricity') group by `hour` order by `hour` desc;
...
AS SELECT
   date_format(`billing_e_day`.`date`,'%xwk%v') AS `week`,round(sum(`billing_e_day`.`off-peak kWh`),3) AS `off-peak kWh`,round(sum(`billing_e_day`.`peak kWh`),3) AS `peak kWh`,round(sum(`billing_e_day`.`total kWh`),3) AS `total kWh`,round(sum(`billing_e_day`.`cost`),2) AS `cost`,round(((sum(`billing_e_day`.`cost`) / sum(`billing_e_day`.`total kWh`)) * 100),2) AS `p/kWh`,group_concat(distinct `billing_e_day`.`tariffs` separator ',') AS `tariffs`,concat(min(`billing_e_day`.`date`),' ~> ',max(`billing_e_day`.`date`)) AS `dates`
